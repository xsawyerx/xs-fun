...
The first line defines that perl (the compiler) should not fetch the
context for every request. This will make the code more efficient at the
cost of being aware of MULTIPLICITY. While this is a somewhat vague
description, it should generally be regarded as, I<unless I'm writing
threaded, this will make my XS code faster without any additional work
from me>, which is good enough for us to add it. It's considered a modern
XS practice.
