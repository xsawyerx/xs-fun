...
When you store a value using either C<hv_store> or C<hv_store_ent>, what Perl
actually does is fetch the value SV for that key and change its PV with a PV
of the new string you wanted to assign. It's actually much faster than
creating a new SV.

So if the SV is read-only, this won't work. So how do we fix it? Well, we can
do various different things:
...
understand this better. Suffice to say that, while it's possible to simply
disable the B<READONLY> restriction and then change the value anyway, this
will cause a massive action-at-a-distance behavior in the future, and we
(and everyone we care about) will hate us forever for this.
