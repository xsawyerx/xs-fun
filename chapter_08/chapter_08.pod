=head1 Chapter 08 - Mapping types
...
to create an IV with the value and return it. If we want to receive an
...
these types (and any other you want) and have Perl's XS preprocessor
add the translation code for you. B<TYPEMAP>s are the mechanism XS uses
to automatically translate different data types.
...
You can read about typemaps in L<perlxstypemap>.
...
with the string C<Ptr>. This means that if we had the line:
...
We also need to make sure we return a C<Audio::Chromaprint> variable.
We change the XS function's beginning to:

    Audio::Chromaprint
    new( const char *class, ... )

Perl will do something quite interesting here. Let's print out what we
got back using L<Devel::Peek>:
...
I<(This might look a bit different, depending on your version of the Perl
interpreter.)>

We have an B<IV> which is an B<RV>, which references an B<PVMG>, which has
