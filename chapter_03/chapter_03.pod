...
we're trying to learn us some XS, don't we?

=head2 Preparation

First of all, we create a new skeleton which has the same structure as the
old but with the new names for files and using a proper namespace.
...
C<newRV_inc> creates a reference from an SV. It expects an SV, so we cast our
HV as one. We return the data back into a new SV.
...
called with the hash reference we created with C<newRV_inc> and the pointer
from C<gv_stashpv>.
