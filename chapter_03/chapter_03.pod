...
it wouldn't use it's object parameter at all, so
...
are okay". We'll cover later how to actually use parameters passed to
...
in your XS code. Perl keeps a count of
"live" values (technically, the most generic form of a Perl value is an C<SV>)
using this count of things that refer to it.
...
"owns" that value. When we create an
C<RV> that points at the C<HV>, the C<RV> should now own its on reference count
to the C<HV>. We can now use C<newRV_inc> to create the C<RV>. That I<inc>rements the hash's
reference count to 2, and both the C<RV> and our code own one reference to it.

C<newRV_noinc> creates an C<RV>
without increasing the reference count. Generally, this means that the C<RV>
is taking ownership of one of the existing references. Why is this important?
We don't plan to hold on to that C<HV> in our code. The C<RV> will fully own
it and we will return the reference, the C<RV> from the XSUB. If that C<RV>
is freed, it will give up its reference (count) to the C<HV> and perl will
free the C<HV> correctly.
